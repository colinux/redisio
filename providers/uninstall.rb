...
action :run do
    ...
    unless new_resource.servers.nil?
      new_resource.servers.each do |server|
        ...
        execute "rm -rf /etc/init.d/redis_#{server_name}" if ::File.exists?("/etc/init.d/redis_#{server_name}")
        execute "rm -rf /etc/init/redis_#{server_name}.conf" if ::File.exists?("/etc/init/redis_#{server_name}.conf")
      end
    end
end
