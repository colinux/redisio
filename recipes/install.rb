...
redis_instances.each do |current_server|
  ...
  if job_control == 'initd'
    service "redis_#{server_name}" do
      start_command "/etc/init.d/redis_#{server_name} start"
      stop_command "/etc/init.d/redis_#{server_name} stop"
      status_command "pgrep -lf 'redis_.*#{server_name}' | grep -v 'sh'"
      restart_command "/etc/init.d/redis_#{server_name} stop && /etc/init.d/redis_#{server_name} start"
